...
int index_load(Index *index) {
    ...
    if (!f) 
        return 0; // If it doesn't exist, we start with an empty index
    ...
    while (index->count < MAX_INDEX_ENTRIES) {
        ...
        // Convert the hex string back to a binary ObjectID
...
int index_save(const Index *index) {
    // 1. Sort the index entries before saving (standard VCS behavior)
    ...
    *sorted = *index;
    ...
    // 4. Flush and sync to ensure data is physically on disk
    fflush(f);
    fsync(fileno(f));
    fclose(f);
    free(sorted);
